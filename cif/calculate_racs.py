...
def main():
    # 指定目标目录
    target_dir = r"M:\RASPA问题\MOF数据库\coremof2024\1\1558030"
    ...
    # 将所有结果保存到 JSON 文件
    output_json = os.path.join(os.path.dirname(__file__), "racs_results-test.json")
    try:
    ...
    # 将所有结果保存到 CSV 文件
    output_csv = os.path.join(os.path.dirname(__file__), "racs_results-test.csv")
    try:
        if results:
            import csv
    ...
    # 如果有错误，保存错误日志
    if errors:
        error_log = os.path.join(os.path.dirname(__file__), "racs_errors.log")
