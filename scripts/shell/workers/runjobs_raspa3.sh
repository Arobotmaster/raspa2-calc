...
: "${RASPA_TOOL_DIR:=}"
: "${RASPA_WORKER_ID:=}"
: "${RASPA_LOCK_STALE_SECONDS:=}"
: "${RASPA_QUEUE_RESCAN_INTERVAL_SECONDS:=}"
: "${RASPA_MSER_ENABLE:=}"
: "${RASPA_MSER_TARGET_CYCLES:=}"
: "${RASPA_MSER_ADD_CYCLES:=}"
: "${RASPA_MSER_MAX_ITER:=}"
: "${RASPA_MSER_UNCERTAINTY:=}"
: "${RASPA_MSER_CONDA_ENV:=}"
: "${SLURM_ARRAY_JOB_ID:=}"
: "${SLURM_ARRAY_TASK_ID:=}"
: "${SLURM_JOB_ID:=}"
: "${PYTHONPATH:=}"
...
wait_for_output_txt() {
    local wait_sec="${RASPA_MSER_WAIT_SEC:-10}"
    local i=0
    while [ "$i" -lt "$wait_sec" ]; do
        if ls output/*.txt >/dev/null 2>&1; then
            return 0
        fi
        sleep 1
        i=$((i + 1))
...
while :; do
    ...
    if [ -d "output" ] && [ "$OUTPUT_FILES_COUNT" -gt 0 ]; then
        ...
        if [ $mser_status -ne 0 ]; then
            ...
            FRAMEWORK_NAME=$(python3 -c "import json; print(json.load(open('simulation.json'))['Systems'][0]['Name'].split('/')[-1])" 2>/dev/null || true)
