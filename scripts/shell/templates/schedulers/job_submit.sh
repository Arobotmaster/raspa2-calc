...
if [ -n "$RASPA_WORK_DIR" ] && [ -d "$RASPA_WORK_DIR" ]; then
    ...
    BASE_DIR="$TARGET_DIR"
    while [ "$BASE_DIR" != "/" ] && [ ! -d "$BASE_DIR/scripts/shell/entrypoints" ]; do
        BASE_DIR="$(dirname "$BASE_DIR")"
    done
    [ -d "$BASE_DIR/scripts/shell/entrypoints" ] || BASE_DIR=""
fi
if [ -z "$BASE_DIR" ]; then
...
TOOL_WORKERS="$TOOL_DIR/scripts/shell/workers"
RASPA_VERSION_LOWER="$(echo "${RASPA_VERSION:-raspa2}" | tr '[:upper:]' '[:lower:]')"
if [ "$RASPA_VERSION_LOWER" = "raspa3" ] && [ -f "$TOOL_WORKERS/runjobs_raspa3.sh" ]; then
  RUNNER="$TOOL_WORKERS/runjobs_raspa3.sh"
elif [ -f "$TOOL_WORKERS/runjobs.sh" ]; then
  RUNNER="$TOOL_WORKERS/runjobs.sh"
elif [ -f "$BASE_DIR/scripts/shell/workers/runjobs.sh" ]; then
  RUNNER="$BASE_DIR/scripts/shell/workers/runjobs.sh"
fi
if [ -z "$RUNNER" ]; then
