...
prepare_target_dir() {
  has_work_marker() {
    local dir="$1"
    if [ -d "$dir/scripts/shell/entrypoints" ]; then
      return 0
    fi
  ...
  # 尝试在当前路径向上寻找工作根目录（templates 或任务标记）
  ...
  if [ ! -f "$CONFIG_SNAPSHOT" ]; then
    for candidate in "$TARGET_DIR/config.yaml" "$BASE_DIR/config.yaml" "$ROOT_DIR/config.yaml"; do
      if [ -f "$candidate" ]; then
